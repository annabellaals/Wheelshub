...
const CreateListing = () => {
  ...
  // Store data about a new vehicle
  const [formData, setFormData] = useState({ year: "", location: "", description: "", transmission: "", condition: "", body_type: "", fuel_type: "", engine_capacity: "", title: "", make: "", model: "", variant: "", mileage: "", asking_price: "" });
  ...
  const handleSubmit = async () => {
    ...
    const dealData = {
      ...
      description: formData.description ? formData.description.toLowerCase() : "",
      engine_capacity: formData.engine_capacity ? formData.engine_capacity.toLowerCase() : "",
    ...
    try {
      ...
      if (response.ok) {
        setSubmitStatus({ show: true, type: "success", message: `Deal created successfully with ID ${data.deal}` });

        // Reset form and navigate to view listing
        setFormData({ title: "", description: "", price: "", location: "", make: "", model: "", year: "", transmission: "", body_type: "", fuel_type: "", mileage: "", condition: "", engine_capacity: "" });
        
        navigate(`/view/${data.deal}`);
      } 
      else {

        const errorMessage = data?.detail || "Failed to create deal. Please try again.";
  ...
  return <Box sx={{ maxWidth: "800px", minWidth: "300px", mx: "auto", p: 3, width: "90%", minHeight: "100vh" }}>
    ...
    <Box sx={{ gap: 2, mb: 4 }}>
      ...
      <Typography level="body2">Give us information about the car and you to create a new listing</Typography>
